...
###------------BEGINNING ITERATION-----------------------------------------------------------
###------------------------------------------------------------------------------------------
for num in range(2,number_till+1):
    for iter in range(len(theta_list)):
        ...
        theta_poi_dict.setdefault(num,{}).setdefault(theta,{})['theta']=theta
        theta_poi_dict[num][theta]['poissons_ratio']=Poisson_rrs
        theta_poi_dict[num][theta]['num=nx=ny']=num
        theta_poi_dict[num][theta]['']

        Aux_odb_object.close()

        #Writing to file
        with open('DispOutputWNum.txt','a') as f:
            fields=['num=nx=ny','theta','poissons_ratio']
            writer=csv.DictWriter(f,fieldnames=fields)
            #writer.writeheader()
            print(theta_poi_dict[num][theta])
            writer.writerow(theta_poi_dict[num][theta])
