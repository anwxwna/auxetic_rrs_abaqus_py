...
#theta_list=np.arange(2,44,5).tolist()
#ln1=15.0
#ln=50.0
#number of unit cells-----------------------------
initn=2
nx=initn        #number of unit cells x direction
ny=initn         #number of unit cells in y direction
...
###------------BEGINNING ITERATION-----------------------------------------------------------
###------------------------------------------------------------------------------------------
for num in range(initn,number_till+1):
    for iter in range(len(theta_list)):
        ...
        ############################################################################
        ##CREATE MODEL--------------------------------------------------------------
        frac,whole=math.modf(theta)
        model_name='rrs'+ '_' +repr(int(whole))+ '_'+repr(int(frac*10)) + '_' +repr(num)
        ...
        right_set=AuxNodes.getByBoundingCylinder((2*ln*nx,0,0),(2*ln*nx,2*ln*ny,0),0.01)
        ...
        top_set=AuxNodes.getByBoundingCylinder((0,2*ln*ny,0),(2*ln*nx,2*ln*ny,0),0.01)
        ...
        def WriteDispLoc(r_node_location,r_node_disp,k,q='r'):
            key1='num' + repr(num)
            if k==0:
            ...
            r_xy[key1]['theta']=theta
            r_xy[key1]['num']= num

            #print('the dict is',r_xy[key1])
            filename='NumVsThetaVsDisp'+ '_' + q + '.csv'
        ...
        for k in range(len(frames)):
            ...
            WriteDispLoc(r_node_location,r_node_disp,k,'r')
            WriteDispLoc(t_node_location,t_node_disp,k,'t')
