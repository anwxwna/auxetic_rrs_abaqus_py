...
omega=5
cycles=5
AuxPattM.PeriodicAmplitude(a_0=1,data=((csin,ccos),),frequency=omega, name='DynAmpSin',start=0.0,timeSpan=STEP)

#Dynamic general step for displacement BC at right edge
AuxPattM.ImplicitDynamicsStep(initialInc=(2*pi/omega)/50.0,maxInc=(2*pi/omega)/50.0,maxNumInc=10000,minInc=0.000002, timePeriod=(2*pi/omega)*cycles, name='Disp_step',previous='Initial',description='Apply Displacement at right boundary',nlgeom=ON)
...
for to in range(len(ft)):
    ...
    #right_disp_point_region=regionToolset.Region(edges=disp_set_right)
    AuxPattM.HistoryOutputRequest(name='top_disp_output' + repr(ri), createStepName= 'Disp_step', variables=('U2',),frequency=1,region=right_disp_point_region,sectionPoints=DEFAULT,rebar=EXCLUDE)
