...
for theta in theta_list:
	...
	pi=math.pi
	omega=1
	applied_pressure=20.0
	AuxPattM.PeriodicAmplitude(a_0=0,data=((csin,ccos),),frequency=omega, name='DynAmpSin',start=0.0,timeSpan=STEP)

	#Dynamic general step for force BC at right edge
	AuxPattM.ImplicitDynamicsStep(alpha=DEFAULT,application=QUASI_STATIC, initialConditions=OFF, initialInc=(1/omega)/50.0,maxInc=(1/omega)/50.0,
									maxNumInc=10000,minInc=0.000002,timePeriod=1/omega, name='Force_step',
									previous='Initial', description='Apply Force at right boundary')
									
	AuxPattM.steps['Force_step'].setValues(nlgeom=ON)
	

	#Create Field Output Request--------------------------------------------------
	AuxPattM.fieldOutputRequests.changeKey(fromName='F-Output-1', toName='Displacements')
	AuxPattM.fieldOutputRequests['Displacements'].setValues(variables=('U','COORD'))
	...
	##Setting FIXED boundary condition
	for x in range(2*nx):
	...
	##APPLY PRESSURE BOUNDARY CONDITION AT RIGHT EGDES-----------------------------------------------------------------------------------
	...
	for r in range(2*ny):
		...
		AuxPattM.Pressure(name='Edge_r'+repr(r),amplitude='DynAmpSin', createStepName='Force_step', distributionType=UNIFORM, 
				, magnitude=applied_pressure, name='Load-1', region= r_edge_region)
	...
	for ri in range(len(fr)):
		force_set_right=AuxInstance.edges.findAt(fr[ri])
		print(force_set_right[0])
		AuxAssembly.Set(edges=disp_set_right, name='force_set_right')
		right_force_point_region=AuxAssembly.sets['force_set_right']
		print(right_force_point_region)
		#right_disp_point_region=regionToolset.Region(edges=disp_set_right)
		AuxPattM.HistoryOutputRequest(name='right_force_output' + repr(ri), createStepName= 'Force_step', variables=('CF1,TF1',),frequency=1,region=right_force_point_region,sectionPoints=DEFAULT,rebar=EXCLUDE)


	#top_disp_point_region=AuxAssembly.sets['disp_set_top']
	for to in range(len(ft)):
		disp_set_top=AuxInstance.edges.findAt(ft[to])
		print(disp_set_top[0])
		AuxAssembly.Set(edges=disp_set_right, name='disp_set_top')
		top_disp_point_region=AuxAssembly.sets['disp_set_top']
		print(top_disp_point_region)
		#right_disp_point_region=regionToolset.Region(edges=disp_set_right)
		AuxPattM.HistoryOutputRequest(name='top_disp_output' + repr(ri), createStepName= 'Force_step', variables=('U2',),frequency=1,region=top_disp_point_region,sectionPoints=DEFAULT,rebar=EXCLUDE)
	...
	#AuxAssembly.Set(name='top_set1',nodes=right_set)
	...
	disp1=Aux_odb_object.steps['Force_step']
	pathname='ForceVsDispDynSim' +repr(theta)
	...
	#for t in range(3):
	#    for s in range(2):
	#        D=disp1.historyRegions.values()[t].historyOutputs[dispkeys[t][s]]
